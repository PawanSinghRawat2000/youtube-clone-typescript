...
import { YOUTUBE_DATA_URL } from "./constants";
...
export const parseData = async (items: any[]) => {
  try {
    ...
    const {
      ...
      `${YOUTUBE_DATA_URL}/channels?part=snippet,contentDetails&id=${channelIds.join(
        ","
      )}&key=${API_KEY}`
    );
    ...
    const {
      ...
      `${YOUTUBE_DATA_URL}/videos?part=contentDetails,statistics&id=${videoIds.join(
        ","
      )}&key=${API_KEY}`
    );
